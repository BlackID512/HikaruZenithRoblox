...
PluginsHint.Text = "Download plugins from the HZ Discord (discord.gg/78ZuWSq)"
...
reference = (function()
	local main = create({
		...
		{114,"TextLabel",{BackgroundColor3=Color3.new(1,1,1),BackgroundTransparency=1,Font=3,Name="Text",Parent={112},Position=UDim2.new(0,8,0,30),Size=UDim2.new(1,-8,0,32),Text="You can join the Discord server to get support with HZ,  and read up on more documentation such as the Plugin API.",TextColor3=Color3.new(1,1,1),TextSize=14,TextWrapped=true,TextXAlignment=0,ZIndex=10,}},
...
function getNow()
	...
	local currentDate = os.date("%d/%m/%Y %H:%M:%S", currentTime)
	local now = currentDate
	return now
end
...
function sendChatWebhook(player, msgtype, message)
...
SaveChatlogs.MouseButton1Down:Connect(function()
	if writefileExploit() then
		if #scroll_2:GetChildren() > 0 then
			...
			local writelogs = '-- Hikaru Zenith Chat logs for "'..placeName..'"\n'
			for _, child in pairs(scroll_2:GetChildren()) do
				writelogs = writelogs..'\n'..child.Text
			end
...
addcmd('keephz', {}, function(args, speaker)
	if queueteleport then
		KeepHiZen = true
		notify('KeepHZ','Hikaru Zenith will now run after you teleport')
		updatesaves()
	else
...
addcmd('unkeephz', {}, function(args, speaker)
	if queueteleport then
		KeepHiZen = false
		notify('KeepHZ','Hikaru Zenith will no longer run after you teleport')
		updatesaves()
	else
		notify('Incompatible Exploit','Your exploit does not support this command (missing queue_on_teleport)')
	end
...
addcmd('hidehz',{},function(args, speaker)
	...
	minimizeNum = 0
	minimizeHolder()
	if not (args[1] and tostring(args[1]) == 'nonotify') then notify('HZ Hidden','You can press the prefix key to access the command bar') end
end)
...
addcmd('joindate',{'jd'},function(args, speaker)
	...
	for i,v in pairs(players) do
		local p = Players[v]
		local user = formatUsername(p)

		local secondsOld = p.AccountAge * 24 * 60 * 60
		local now = os.time()
		-- local dateJoined  = p.Name .. " joined: " .. os.date("%d/%m/%Y", now - secondsOld)
		local dateJoined  = user .. " joined: " .. os.date("%d/%m/%Y", now - secondsOld)

		table.insert(dates, dateJoined)
	end
	notify('Join Date (Day/Month/Year)',table.concat(dates, ',\n'))
end)
...
addcmd('antigameplaypaused',{'agp'},function(args, speaker)
	loadstring(game:HttpGet('https://raw.githubusercontent.com/BlackID512/HikaruZenithRoblox/master/AntiGameplayPaused.lua'))()
end)
