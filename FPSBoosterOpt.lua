...
local function defNotify(title,msg,duration)
...
local function confirmNotify(title,msg,buttonMsg,duration)
    game.StarterGui:SetCore("SendNotification",{
		Title = title,
		Text = msg,
		-- Duration = 5,
...
if _G.SendNotifications then
	...
	Text = "Initiating..."
	Duration = 3
	defNotify(Title,Text,Duration)
end
coroutine.wrap(pcall)(function()
    if (_G.Settings["Low Water Graphics"] or (_G.Settings.Other and _G.Settings.Other["Low Water Graphics"])) then
        local terrain = workspace:FindFirstChildOfClass("Terrain")
		local Title = "FPS Booster"
        if not terrain then
            repeat
                task.wait()
        ...
        if sethiddenproperty then
			...
			Duration = 5
			Button1 = "Okay"
			confirmNotify(Title,Text,Button1,Duration)
    ...
    if _G.Settings.Other["ClearNilInstances"] then
        if getnilinstances then
            ...
            if _G.SendNotifications then
				...
				Duration = 5
				-- Button1 = "Okay"
				confirmNotify(Title,Text,Duration)
            end
        else
